from models import ReceiverAgreementList,ReceiverAgreement, Base,engine,SessionLocal
import json
import hashlib
...
def get_total():
    db = SessionLocal()
    items = db.query(ReceiverAgreementList).all()
    total = len(items)
    return total
...
async def extraction_task(procedure_name: str,lock: asyncio.Lock):
    ...
    try:
        ...
        logger.info(f"📦 Found {total} ReceiverAgreementList items in the database.")
